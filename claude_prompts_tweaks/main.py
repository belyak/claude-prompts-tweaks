...
@click.option(
    "--format",
    "output_format",
    type=click.Choice(["json", "md", "txt"]),
    default="json",
    help="Output format",
)
def analyze(
    json_file: Union[str, Path], output: Optional[Union[str, Path]], output_format: str
) -> None:
...
def search(
    json_file: Union[str, Path], pattern: Optional[str], category: Optional[str]
) -> None:
...
def extract(json_file: Union[str, Path], output: Optional[Union[str, Path]]) -> None:
...
def _analyze_prompts(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if all_prompts:
        ...
        stats["longest_prompt"] = (
            str(all_prompts[longest_idx])[:200] + "..."
            if len(str(all_prompts[longest_idx])) > 200
            else str(all_prompts[longest_idx])
        )
        stats["shortest_prompt"] = (
            str(all_prompts[shortest_idx])[:200] + "..."
            if len(str(all_prompts[shortest_idx])) > 200
            else str(all_prompts[shortest_idx])
        )
...
def _search_prompts(
    data: Dict[str, Any], pattern: Optional[str], category: Optional[str]
) -> List[Dict[str, Any]]:
